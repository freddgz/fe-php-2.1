...
class Factura extends CI_Controller {
    ...
    protected $username="2055705569100DL2RGZ";
    protected $password="l2E45678";
    ...
    protected $url_guia="";
    protected $url_guia_produccion="https://e-guiaremision.sunat.gob.pe/ol-ti-itemision-guia-gem/billService?wsdl";
    protected $url_guia_beta="https://e-beta.sunat.gob.pe/ol-ti-itemision-guia-gem-beta/billService?wsdl";
...
public function resumen(){
    try{
        ...
        $objKey->loadKey('perno.key', true);
        $objDSig->sign($objKey);
        // Agregue la clave pública asociada a la firma
        $objDSig->add509Cert(file_get_contents('perno.cer'), true, false, array('subjectName' => true)); // array('issuerSerial' => true, 'subjectName' => true));
        ...
        json_output(200,array('status'=>'200', 'ticket'=>$respuesta));
    }
    } catch (Exception $e) {
        json_output(500,array('status'=>500, 'mensaje'=>$e->getMessage(), 'mensaje-detalle'=>$e->getTraceAsString()));
    }
...
public function sunat(){
    ...
    if($method!='POST'){
        ...
        */
        ...
        $InvoiceTypeCode = $dom->createElement('cbc:InvoiceTypeCode', $doc_tipo_documento);
        ...
        */
        ...
        $zip->close();
            ...
            $codigo = $r_doc->getElementsByTagName('ResponseCode')->item(0)->nodeValue;
            $respuesta = $r_doc->getElementsByTagName('Description')->item(0)->nodeValue;
            ...
            */
            ...
            $Invoice->appendChild($Signature);
                ...
                $Signature->appendChild($DigitalSignatureAttachment);
                    $ExternalReference = $dom->createElement('cac:ExternalReference');
                    $DigitalSignatureAttachment->appendChild($ExternalReference);
                        $ExternalReference->appendChild($dom->createElement('cbc:URI', '#'.$doc_numero));
    ...
    try{
       $method = $_SERVER['REQUEST_METHOD'];
        ...
        if($method!='POST'){
            ...
            $doc_ref_numero = $cab['doc_ref_numero'];
            $doc_ref_tipo = $cab['doc_ref_tipo'];
            ...
            $Invoice = $dom->createElement('CreditNote');
            $dom->appendChild($Invoice);
            ...
            $Invoice->appendChild($DiscrepancyResponse);
                $DiscrepancyResponse->appendChild($dom->createElement('cbc:ReferenceID',$doc_ref_numero));
                $DiscrepancyResponse->appendChild($dom->createElement('cbc:ResponseCode','01'));
                $DiscrepancyResponse->appendChild($dom->createElement('cbc:Description','ANULACION DE LA OPRACION'));
            ...
            $Invoice->appendChild($BillingReference);
                ...
                $BillingReference->appendChild($InvoiceDocumentReference);
                    $InvoiceDocumentReference->appendChild($dom->createElement('cbc:ID',$doc_ref_numero));
                    $InvoiceDocumentReference->appendChild($dom->createElement('cbc:DocumentTypeCode',$doc_ref_tipo));
                ...
                $AccountingSupplierParty->appendChild($Party);                
                    ...
                    $Party->appendChild($PartyLegalEntity);
                        ...
                        $PartyLegalEntity->appendChild($RegistrationAddress);
                            ...
                            $AddressTypeCode = $dom->createElement('cbc:AddressTypeCode','0001');
                            $RegistrationAddress->appendChild($AddressTypeCode);
            ...
            foreach ($cab['detalle'] as $item) {
                ...
                $Invoice->appendChild($InvoiceLine);
                    $InvoiceLine->appendChild($dom->createElement('cbc:ID', $item['orden']));
                    $InvoicedQuantity = $dom->createElement('cbc:CreditedQuantity', $item['cantidad']);
                    $InvoiceLine->appendChild($InvoicedQuantity);
                        $InvoicedQuantity->setAttribute('unitCode', $item['unidad']);
    ...
    try{
       $method = $_SERVER['REQUEST_METHOD'];
        ...
        if($method!='POST'){
            ...
            $Invoice = $dom->createElement('DebitNote');
            $dom->appendChild($Invoice);
            ...
            //bloque 6
            $RequestedMonetaryTotal = $dom->createElement('cac:RequestedMonetaryTotal');
            ...
            foreach ($cab['detalle'] as $item) {
                ...
                $Invoice->appendChild($InvoiceLine);
                    $InvoiceLine->appendChild($dom->createElement('cbc:ID', $item['orden']));
                    $InvoicedQuantity = $dom->createElement('cbc:DebitedQuantity', $item['cantidad']);
                    $InvoiceLine->appendChild($InvoicedQuantity);
                        $InvoicedQuantity->setAttribute('unitCode', $item['unidad']);
    ...
    try{
       $method = $_SERVER['REQUEST_METHOD'];
        ...
        if($method!='POST'){
            ...
            $ship_cod_motivo = $cab['ship_cod_motivo'];
            $ship_descripcion = $cab['ship_descripcion'];
            $ship_peso = $cab['ship_peso'];
            $ship_cantidad = $cab['ship_cantidad'];
            $ship_modalidad = $cab['ship_modalidad'];

            $ship_origen_fecha = $cab['ship_origen_fecha'];
            $ship_origen_ubigeo = $cab['ship_origen_ubigeo'];
            $ship_origen_direccion = $cab['ship_origen_direccion'];
            $ship_destino_ubigeo = $cab['ship_destino_ubigeo'];
            $ship_destino_direccion = $cab['ship_destino_direccion'];

            $trans_ruc = $cab['trans_ruc'];
            $trans_descripcion = $cab['trans_descripcion'];
            ...
            $Invoice = $dom->createElement('DespatchAdvice');
            $dom->appendChild($Invoice);
            ...
            $Invoice->appendChild($dom->createElement('cbc:DespatchAdviceTypeCode', $doc_tipo_documento));//09
            
        
            //$Note1 = $dom->createElement('cbc:Note', 'una nota');
            //$Invoice->appendChild($Note1);

            //bloque3 cac:AccountingSupplierParty
            $DespatchSupplierParty = $dom->createElement('cac:DespatchSupplierParty');
            $Invoice->appendChild($DespatchSupplierParty);
                $CustomerAssignedAccountID = $dom->createElement('cbc:CustomerAssignedAccountID',$emp_ruc);
                $DespatchSupplierParty->appendChild($CustomerAssignedAccountID);
                    $CustomerAssignedAccountID->setAttribute('schemeID','6');
                $Party = $dom->createElement('cac:Party');
                $DespatchSupplierParty->appendChild($Party); 
                $PartyLegalEntity = $dom->createElement('cac:PartyLegalEntity');
                    $Party->appendChild($PartyLegalEntity);
                        $PartyLegalEntity->appendChild($dom->createElement('cbc:RegistrationName', $emp_razonsocial));
                        
            //bloque 4  
            $DeliveryCustomerParty = $dom->createElement('cac:DeliveryCustomerParty');
            $Invoice->appendChild($DeliveryCustomerParty);
                $CustomerAssignedAccountID = $dom->createElement('cbc:CustomerAssignedAccountID',$cli_numero);
                $DeliveryCustomerParty->appendChild($CustomerAssignedAccountID);
                    $CustomerAssignedAccountID->setAttribute('schemeID',$cli_tipo_documento);
                $Party = $dom->createElement('cac:Party');
                $DeliveryCustomerParty->appendChild($Party); 
                $PartyLegalEntity = $dom->createElement('cac:PartyLegalEntity');
                    $Party->appendChild($PartyLegalEntity);
                        $PartyLegalEntity->appendChild($dom->createElement('cbc:RegistrationName', $cli_nombre));
            
            $Shipment = $dom->createElement('cac:Shipment');
            $Invoice->appendChild($Shipment);
                $Shipment->appendChild($dom->createElement('cbc:ID','1'));
                $Shipment->appendChild($dom->createElement('cbc:HandlingCode',$ship_cod_motivo));
                $Shipment->appendChild($dom->createElement('cbc:Information',$ship_descripcion));
                $GrossWeightMeasure = $dom->createElement('cbc:GrossWeightMeasure',$ship_peso);
                $Shipment->appendChild($GrossWeightMeasure);
                    $GrossWeightMeasure->setAttribute('unitCode','KGM');
                $Shipment->appendChild($dom->createElement('cbc:TotalTransportHandlingUnitQuantity',$ship_cantidad));
                $ShipmentStage = $dom->createElement('cac:ShipmentStage');
                $Shipment->appendChild($ShipmentStage);
                    $ShipmentStage->appendChild($dom->createElement('cbc:TransportModeCode',$ship_modalidad));
                    $TransitPeriod = $dom->createElement('cac:TransitPeriod');
                    $ShipmentStage->appendChild($TransitPeriod);
                        $TransitPeriod->appendChild($dom->createElement('cbc:StartDate',$ship_origen_fecha));
                    ...
                    $ShipmentStage->appendChild($CarrierParty);
                        ...
                        $CarrierParty->appendChild($PartyIdentification);
                            $ID = $dom->createElement('cbc:ID',$trans_ruc);
                            $PartyIdentification->appendChild($ID);
                                $ID->setAttribute('schemeID','6');
                        $PartyName = $dom->createElement('cac:PartyName');
                        $CarrierParty->appendChild($PartyName);
                            $Name = $dom->createElement('cbc:Name',$trans_descripcion);
                            $PartyName->appendChild($Name);
                $Delivery = $dom->createElement('cac:Delivery');
                $Shipment->appendChild($Delivery);
                    $DeliveryAddress = $dom->createElement('cac:DeliveryAddress');
                    $Delivery->appendChild($DeliveryAddress);
                        $DeliveryAddress->appendChild($dom->createElement('cbc:ID',$ship_destino_ubigeo));
                        $DeliveryAddress->appendChild($dom->createElement('cbc:StreetName',$ship_destino_direccion));
                $OriginAddress = $dom->createElement('cac:OriginAddress');
                $Shipment->appendChild($OriginAddress);
                    $OriginAddress->appendChild($dom->createElement('cbc:ID',$ship_origen_ubigeo));
                    $OriginAddress->appendChild($dom->createElement('cbc:StreetName',$ship_origen_direccion));
            ...
            foreach ($cab['detalle'] as $item) {
                ...
                $InvoiceLine = $dom->createElement('cac:DespatchLine');
                $Invoice->appendChild($InvoiceLine);
                    $InvoiceLine->appendChild($dom->createElement('cbc:ID', $item['orden']));
                    $InvoicedQuantity = $dom->createElement('cbc:DeliveredQuantity', $item['cantidad']);
                    $InvoicedQuantity->setAttribute('unitCode',$item['unidad']);
                    $InvoiceLine->appendChild($InvoicedQuantity);

                    $OrderLineReference = $dom->createElement('cac:OrderLineReference');
                    $InvoiceLine->appendChild($OrderLineReference);
                    $OrderLineReference->appendChild($dom->createElement('cbc:LineID',$item['orden']));

                    $Item = $dom->createElement('cac:Item');
                    $InvoiceLine->appendChild($Item);
                        $Item->appendChild($dom->createElement('cbc:Name',$item['descripcion']));
                        $SellersItemIdentification = $dom->createElement('cac:SellersItemIdentification');
                        $SellersItemIdentification->appendChild($dom->createElement('cbc:ID',$item['codigo']));
            }
        ...
        $params=array('url'=>$this->url_guia);
        $this->load->library('Feedsoap',$params);
        //$feedsoap = new Feedsoap();
        $this->feedsoap->SoapClientCall($XMLString); 
        $this->feedsoap->__call("sendBill", array(), array());
